...
def run(issues, GH_TOKEN, reponame):
    ...
    for j in issues:
        ...
        if len(extracted_data) == 1:
            ...
            url = 'https://api.github.com/search/issues?q='+commit_id+'+type:pr+repo:'+reponame  # Ignore PycodestyleBear (E501)
            ...
            a = r.json()
            if a['total_count'] > 0:
                b = r.json()['items'][0]['state']  # check PR is open or closed.
                if b == 'open':
                    ...
                    c = requests.get(url, headers={'Authentication':
                    'token '+GH_TOKEN})
                    d = c.json()
                    if d['state'] == 'pending' or \
                        d['state'] == 'success':
